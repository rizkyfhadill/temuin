...
export function HowItWorks() {
  return (
    <section id="cara-kerja" className="scroll-mt-20 border-b border-border py-16 md:py-24">
      <div className="container">
        <div className="mx-auto max-w-2xl text-center">
          <h2 className="text-3xl font-bold tracking-tight sm:text-4xl">Cara Kerja Temuin</h2>
          <p className="mt-3 text-muted-foreground">
            Alur yang jelas, sederhana, dan mudah dipahami pengguna baru.
          </p>
        </div>
        <RevealGroup className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-5" stagger={0.07}>
          {STEPS.map((s, i) => (
            <RevealItem key={s.title}>
              <div className="group relative h-full rounded-xl border border-border bg-card p-5 card-interactive">
                <span className="absolute right-4 top-4 text-3xl font-black text-border/70">{i + 1}</span>
                <div className="grid size-11 place-items-center rounded-xl bg-primary/10 text-primary transition-transform duration-300 group-hover:scale-110 group-hover:bg-primary/15">
