...
export function CreateReportForm({ categories }: { categories: Category[] }) {
  ...
  return (
    <div className="mx-auto max-w-2xl px-2 sm:px-0">
      {/* Stepper */}
      <ol className="mb-6 sm:mb-8 flex items-center justify-between">
        {STEPS.map((s, i) => (
          <li key={s} className="flex flex-1 items-center">
            <div className="flex items-center gap-1 sm:gap-2">
              <span
                className={cn(
                  "grid size-7 sm:size-8 place-items-center rounded-full text-xs sm:text-sm font-semibold",
                  i <= step ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"
                )}
              >
                {i < step ? <Check className="size-3 sm:size-4" /> : i + 1}
              </span>
              <span className={cn("hidden text-xs sm:text-sm font-medium lg:inline", i <= step ? "text-foreground" : "text-muted-foreground")}>
                {s}
              </span>
            </div>
            {i < STEPS.length - 1 && <span className={cn("mx-1 sm:mx-3 h-px flex-1", i < step ? "bg-primary" : "bg-border")} />}
          </li>
        ))}
      </ol>
      ...
      <Card className="p-4 sm:p-6">
        ...
        {step === 0 && (
          <div className="space-y-4">
            <h2 className="text-base sm:text-lg font-semibold">Apa yang ingin kamu laporkan?</h2>
            <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2">
              {([
                ...
                <button
                  ...
                  onClick={() => setType(o.v)}
                  className={cn(
                    "rounded-lg sm:rounded-xl border p-4 sm:p-5 text-left transition-colors",
                    type === o.v ? "border-primary bg-primary/5" : "border-border hover:bg-accent"
                  )}
                >
                  <o.icon className={cn("size-6 sm:size-8", type === o.v ? "text-primary" : "text-muted-foreground")} />
                  <p className="mt-2 sm:mt-3 font-semibold text-sm sm:text-base">{o.title}</p>
        ...
        {step === 1 && (
          <div className="space-y-4">
            <h2 className="text-base sm:text-lg font-semibold">Unggah Foto — AI akan menganalisis</h2>
            <label className="flex aspect-[4/3] cursor-pointer flex-col items-center justify-center gap-2 rounded-lg sm:rounded-xl border-2 border-dashed border-border bg-muted/40 text-muted-foreground hover:bg-accent">
              {preview ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={preview} alt="preview" className="h-full w-full rounded-lg sm:rounded-xl object-cover" />
              ) : (
                <>
                  <Camera className="size-6 sm:size-8" />
                  <span className="text-xs sm:text-sm">Klik untuk unggah foto</span>
                </>
              )}
              <input
                type="file"
            ...
            {analysis && !analyzing && (
              <div className="rounded-lg border border-primary/20 bg-primary/5 p-3 text-xs sm:text-sm">
        ...
        {step === 3 && (
          <div className="space-y-4">
            <h2 className="text-base sm:text-lg font-semibold">Pratinjau Laporan</h2>
            <div className="overflow-hidden rounded-lg sm:rounded-xl border border-border">
              ...
              <div className="space-y-2 p-3 sm:p-4">
                <Badge variant={type === "lost" ? "destructive" : "success"} className="text-xs sm:text-sm">
                  {type === "lost" ? "Barang Hilang" : "Barang Ditemukan"}
                </Badge>
                <h3 className="text-base sm:text-lg font-bold">{form.title || "—"}</h3>
        ...
        <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          {step < 3 ? (
            ...
            <Button onClick={submit} disabled={submitting || !form.title || !form.category_id} className="w-full sm:w-auto">
